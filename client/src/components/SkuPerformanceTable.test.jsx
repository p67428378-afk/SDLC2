...
describe("SkuPerformanceTable Component", () => {
  ...
  it("renders table headers and all SKU rows including bottom rows", () => {
    ...
    expect(screen.getByText("SKU-10112")).toBeInTheDocument();
    expect(screen.getByText("DG Cheddar Cheese Puffs 8oz")).toBeInTheDocument();
  });

  it("filters SKUs based on search input", () => {
    render(<SkuPerformanceTable skus={mockSkus} loading={false} />);
    const searchInput = screen.getByLabelText("Search SKU or product");
    fireEvent.change(searchInput, { target: { value: "Cheese Puffs" } });

    expect(screen.getByText("DG Cheddar Cheese Puffs 8oz")).toBeInTheDocument();
    expect(
      screen.queryByText("DG Value Pretzels 12oz"),
