...
class PaymentOrchestrationService:
    ...
    def execute_payment(
        ...
        if existing_payment:
            source_bal = (
                self.fiserv_service.get_available_balance(source_account_id) or 0.0
            )
            mort_details = self.cenlar_service.get_mortgage_details(
                user.cenlar_customer_id, mortgage_account_id
            )
        ...
        accounts = self.fiserv_service.get_accounts(user.fiserv_cif)
        source_acc = next(
            (acc for acc in accounts if acc["id"] == source_account_id), None
        )
        if not source_acc:
            raise HTTPException(
                status_code=400,
        ...
        if (
            source_acc["type"] not in ("DDA", "Savings")
            or source_acc["status"] != "Active"
        ):
            raise HTTPException(
                status_code=400, detail="Source account is not eligible for payments"
            )
        ...
        if not mort_acc:
            raise HTTPException(
                status_code=400,
                detail="Mortgage account not found or does not belong to user",
            )
        ...
        if not debit_success:
            raise HTTPException(
                status_code=400, detail="Failed to debit source account"
            )

        # 3. Post to Cenlar
        cenlar_success = False
        try:
            cenlar_success = self.cenlar_service.process_payment(
                mortgage_account_id, amount
            )
        except Exception:
            pass
        ...
        if not cenlar_success:
            ...
            raise HTTPException(
                status_code=400, detail="Failed to post payment to mortgage servicer"
            )
        ...
        updated_source_bal = self.fiserv_service.get_available_balance(
            source_account_id
        )
        updated_mort_details = self.cenlar_service.get_mortgage_details(
            user.cenlar_customer_id, mortgage_account_id
        )
        updated_mort_bal = (
            updated_mort_details["principal_balance"] if updated_mort_details else 0.0
        )
    ...
    def schedule_payment(
        ...
        try:
            ...
            db.rollback()
            raise HTTPException(
                status_code=500, detail=f"Failed to schedule payment: {str(e)}"
            )
    ...
    def cancel_scheduled_payment(
        self, db: Session, user: User, payment_id: str
    ) -> bool:
        payment = (
            db.query(ScheduledPayment)
            .filter(
                ScheduledPayment.id == payment_id, ScheduledPayment.user_id == user.id
            )
            .first()
        )
        if not payment:
        ...
        if payment.status != "PENDING":
            raise HTTPException(
                status_code=400,
                detail="Only pending scheduled payments can be cancelled",
            )
        ...
        try:
            ...
            db.rollback()
            raise HTTPException(
                status_code=500, detail=f"Failed to cancel scheduled payment: {str(e)}"
            )
